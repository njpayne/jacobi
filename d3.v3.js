d3 = function () {
    ...
    function d3_geo_areaRingStart() {
        varÎ»00, Ï†00, Î»0, cosÏ†0, sinÏ†0;
        d3_geo_area.point = function (Î», Ï†) {
            d3_geo_area.point = nextPoint;Î»0 = (Î»00 = Î») * d3_radians, cosÏ†0 = Math.cos(Ï† = (Ï†00 = Ï†) * d3_radians / 2 + Ï€ / 4),
            sinÏ†0 = Math.sin(Ï†);
        };

        function nextPoint(Î», Ï†) {Î» *= d3_radians;Ï† = Ï† * d3_radians / 2 + Ï€ / 4;
            var dÎ» = Î» - Î»0,
                cosÏ† = Math.cos(Ï†),
                sinÏ† = Math.sin(Ï†),
                k = sinÏ†0 * sinÏ†,
                u = cosÏ†0 * cosÏ† + k * Math.cos(dÎ»),
                v = k * Math.sin(dÎ»);
    ...
    d3.geo.bounds = function () {
        varÎ»0, Ï†0, Î»1, Ï†1, Î»_, Î»__, Ï†__, p0, dÎ»Sum, ranges, range;
        var bound = {
            point: point,
            lineStart: lineStart,
            lineEnd: lineEnd,
            ...
            polygonEnd: function () {
                ...
                if (d3_geo_areaRingSum < 0)Î»0 = -(Î»1 = 180), Ï†0 = -(Ï†1 = 90);
                else if (dÎ»Sum > Îµ)Ï†1 = 90;
                else if (dÎ»Sum < -Îµ)Ï†0 = -90;
        ...
        function linePoint(Î», Ï†) {
            ...
            if (p0) {
                ...
                var dÎ» = Î» - Î»_,
                    ...
                    antimeridian = Math.abs(dÎ») > 180;
                if (antimeridian ^ (s * Î»_ < Î»i && Î»i < s * Î»)) {
                    varÏ†i = inflection[1] * d3_degrees;
                    if (Ï†i > Ï†1)Ï†1 = Ï†i;
        ...
        return function (feature) {Ï†1 = Î»1 = -(Î»0 = Ï†0 = Infinity);
            ranges = [];
            ...
            ranges = range = null;
            returnÎ»0 === Infinity || Ï†0 === Infinity ? [
                [NaN, NaN],
                [NaN, NaN]
            ] : [
                [Î»0, Ï†0],
    ...
    function d3_geo_centroidPoint(Î», Ï†) {Î» *= d3_radians;
    ...
    function d3_geo_centroidLineStart() {
        var x0, y0, z0;
        d3_geo_centroid.point = function (Î», Ï†) {Î» *= d3_radians;
            var cosÏ† = Math.cos(Ï† *= d3_radians);
            x0 = cosÏ† * Math.cos(Î»);
            y0 = cosÏ† * Math.sin(Î»);
            z0 = Math.sin(Ï†);
        ...
        function nextPoint(Î», Ï†) {Î» *= d3_radians;
            var cosÏ† = Math.cos(Ï† *= d3_radians),
                x = cosÏ† * Math.cos(Î»),
                y = cosÏ† * Math.sin(Î»),
                z = Math.sin(Ï†),
    ...
    function d3_geo_centroidRingStart() {
        varÎ»00, Ï†00, x0, y0, z0;
        d3_geo_centroid.point = function (Î», Ï†) {Î»00 = Î», Ï†00 = Ï†;
            d3_geo_centroid.point = nextPoint;Î» *= d3_radians;
            var cosÏ† = Math.cos(Ï† *= d3_radians);
            x0 = cosÏ† * Math.cos(Î»);
            y0 = cosÏ† * Math.sin(Î»);
            z0 = Math.sin(Ï†);
        ...
        for (var i = 0, n = polygon.length; i < n; ++i) {
            ...
            while (true) {
                ...
                if (antimeridian ^ Î»0 >= meridian ^ Î» >= meridian) {
                    ...
                    varÏ†arc = (antimeridian ^ dÎ» >= 0 ? -1 : 1) * d3_asin(intersection[2]);
                    if (parallel > Ï†arc) {
                        winding += antimeridian ^ dÎ» >= 0 ? 1 : -1;
                    }
                }
                if (!j++) break;Î»0 = Î», sinÏ†0 = sinÏ†, cosÏ†0 = cosÏ†, point0 = point;
            }
        ...
        return {
            ...
            point: function (Î»1, Ï†1) {
                ...
                if (Math.abs(dÎ» - Ï€) < Îµ) {
                    ...
                    if (Math.abs(Î»1 - sÎ»1) < Îµ)Î»1 -= sÎ»1 * Îµ;Ï†0 = d3_geo_clipAntimeridianIntersect(Î»0, Ï†0, Î»1, Ï†1);
                    listener.point(sÎ»0, Ï†0);
                    listener.lineEnd();
                    listener.lineStart();
                    listener.point(sÎ»1, Ï†0);
    ...
    function d3_geo_clipAntimeridianInterpolate(from, to, direction, listener) {
        ...
        if (from == null) {Ï† = direction * Ï€ / 2;
            ...
            var s = (from[0] < to[0] ? 1 : -1) * Ï€;Ï† = direction * s / 2;
            listener.point(-s, Ï†);
            listener.point(0, Ï†);
            listener.point(s, Ï†);
        } else {
    ...
    function d3_geo_clipCircle(radius) {
        ...
        function intersect(a, b, two) {
            ...
            varÎ´Î» = Î»1 - Î»0,
                polar = Math.abs(Î´Î» - Ï€) < Îµ,
                meridian = polar || Î´Î» < Îµ;
    ...
    function d3_geo_conicEqualArea(Ï†0, Ï†1) {
        ...
        function forward(Î», Ï†) {
            varÏ = Math.sqrt(C - 2 * n * Math.sin(Ï†)) / n;
    ...
    function d3_geo_resample(project) {
        ...
        function resample(stream) {
            varÎ»00, Ï†00, x00, y00, a00, b00, c00, Î»0, x0, y0, a0, b0, c0;
            var resample = {
                point: point,
                lineStart: lineStart,
                lineEnd: lineEnd,
        ...
        function resampleLineTo(x0, y0, Î»0, a0, b0, c0, x1, y1, Î»1, a1, b1, c1, depth, stream) {
            ...
            if (d2 > 4 * Î´2 && depth--) {
                var a = a0 + a1,
                    ...
                    Ï†2 = Math.asin(c /= m),
                    Î»2 = Math.abs(Math.abs(c) - 1) < Îµ ? (Î»0 + Î»1) / 2 : Math.atan2(b, a),
                    p = project(Î»2, Ï†2),
                    x2 = p[0],
                    y2 = p[1],
                    dx2 = x2 - x0,
    ...
    function d3_geo_projectionMutator(projectAt) {
        var project, rotate, projectRotate, projectResample = d3_geo_resample(function (x, y) {
            ...
            Î´Ï† = 0,
            Î´Î³ = 0,
            Î´x, Î´y, preclip = d3_geo_clipAntimeridian,
        ...
        projection.center = function (_) {
            if (!arguments.length) return [Î» * d3_degrees, Ï† * d3_degrees];Î» = _[0] % 360 * d3_radians;Ï† = _[1] % 360 * d3_radians;
            return reset();
        };
        projection.rotate = function (_) {
            if (!arguments.length) return [Î´Î» * d3_degrees, Î´Ï† * d3_degrees, Î´Î³ * d3_degrees];Î´Î» = _[0] % 360 * d3_radians;Î´Ï† = _[1] % 360 * d3_radians;Î´Î³ = _.length > 2 ? _[2] % 360 * d3_radians : 0;
            return reset();
        };
        ...
        function reset() {
            ...
            var center = project(Î», Ï†);Î´x = x - center[0] * k;Î´y = y + center[1] * k;
            return invalidate();
        }
    ...
    function d3_geo_rotation(Î´Î», Î´Ï†, Î´Î³) {
        returnÎ´Î» ? Î´Ï† || Î´Î³ ? d3_geo_compose(d3_geo_rotationÎ» (Î´Î»), d3_geo_rotationÏ†Î³ (Î´Ï†, Î´Î³)) : d3_geo_rotationÎ» (Î´Î») : Î´Ï† || Î´Î³ ? d3_geo_rotationÏ†Î³ (Î´Ï†, Î´Î³) : d3_geo_equirectangular;
    }

    function d3_geo_forwardRotationÎ» (Î´Î») {
        return function (Î», Ï†) {
            returnÎ» += Î´Î», [Î» > Ï€ ? Î» - 2 * Ï€ : Î» < -Ï€ ? Î» + 2 * Ï€ : Î», Ï†];
        };
    }
    ...
    function d3_geo_lengthLineStart() {
        varÎ»0, sinÏ†0, cosÏ†0;
        d3_geo_length.point = function (Î», Ï†) {Î»0 = Î» * d3_radians, sinÏ†0 = Math.sin(Ï† *= d3_radians), cosÏ†0 = Math.cos(Ï†);
    ...
    function d3_geo_conicConformal(Ï†0, Ï†1) {
        ...
        function forward(Î», Ï†) {
            varÏ = Math.abs(Math.abs(Ï†) - Ï€ / 2) < Îµ ? 0 : F / Math.pow(t(Ï†), n);
    ...
    d3.random = {
        normal: function (Âµ, Ïƒ) {
            ...
            return function () {
                ...
                returnÂµ + Ïƒ * x * Math.sqrt(-2 * Math.log(r) / r);
            };
        },
        logNormal: function () {
